...
戦闘画面下段の手札領域では現在「カードA / カードB / カードC」というテキストのプレースホルダが表示されている。これを `frontend/public/sprites/cards/` 配下のカード画像（`attack.png` / `guard.png` / `heal.png`）に差し替え、同時に「手札全体のレイアウト」と「個別カードの見た目」を別コンポーネントに分離する。合わせて、カードの `power` はステージ依存パラメータとして `stages.json` 側で定義する構造に変更する。現行の `cards.json` は廃止し、必要なメタデータはステージ定義に集約する。
...
#### 受け入れ基準
1. WHEN 戦闘画面が手札を描画する THEN the system SHALL `/sprites/cards/<id>.png` を `<img>` で表示する
