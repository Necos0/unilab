 ...
 *                                                                            
 * `battleStore.enemyDamageEvents` 配列を購読し、各要素を `<span>` として
 * マップする。各 `<span>` は CSS の `@keyframes damageFloat` で              
 * 上方向にスライドしながらフェードアウトし、`onAnimationEnd` で              
 * `dismissEnemyDamageEvent(id)` を呼んで自身を配列から取り除く（自走 unmount）。  
 *                                                                            
...
function DamageFloater() {
    const enemyDamageEvents = useBattleStore((s) => s.enemyDamageEvents);
    const dismiss = useBattleStore((s) => s.dismissEnemyDamageEvent);

    return (
        <div className={styles.layer}>
            {enemyDamageEvents.map((e) => (
                <span
                    key={e.id}
