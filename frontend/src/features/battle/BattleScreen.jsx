 ...
 *   - 上段: 敵スプライト（`EnemySprite` によるアニメーション描画）
 *   - 中段: フローチャート領域（React Flow）
 *   - 下段: HP と手札カード領域
 * 現在ステージの `enemyId` を `EnemySprite` に渡し、`enemies.json`
 * の定義に基づいた idle アニメーションを表示する。中段は `stages.json`
 * の先頭ステージを `FlowchartArea` に渡して空きスロットとエッジを
 * 描画する。下段は現時点ではプレースホルダ表示で、後続の実装で
 * 実際のカード UI を埋め込む。スタイルは同ディレクトリの
